 ...
 * POST /api/leads
 *
 * Handles both forms that exist in the front end:
 *  - contacts.html  → form_type = "contact_short"   (name, phone, message)
 *  - request.html   → form_type = "measurement_request" (name, phone, email,
 *                      project_type, area, message)
 *
 * Both write into the single `leads` table defined in schema.sql.
 *
 * Consent: neither form in the current markup has a real checkbox — the
 * text under the submit button ("Нажимая кнопку, вы соглашаетесь...")
 * implies consent-by-submission. We record consent_given = TRUE on that
 * basis. If a real checkbox is added later, read it here instead of
 * hardcoding TRUE, and reject the request when it's unchecked.
 */
function createLead(): never
{
    $body = requestBody();
